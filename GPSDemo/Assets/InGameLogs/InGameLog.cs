...
public class InGameLog : MonoBehaviour
{
    ...
    public static void Create(int max = 1000)
    {
        GameObject go = new GameObject("InGameLog");
        InGameLog log = go.AddComponent<InGameLog>();
        log.maxAllowedLog = max;
    }
#if UNITY_EDITOR
    [UnityEditor.MenuItem("Assets/InGameLog/Add InGameLog")]
    public static void AddInGameLog()
    {
        /*
		...
		MonoBehaviour mb = GameObject.FindObjectOfType<InGameLog>();
		if(mb==null){
			main.gameObject.AddComponent<InGameLog>();
		}
        */
        ...
        if (mb == null)
        {
            mainObj.AddComponent<InGameLog>();
        }
    }

    [UnityEditor.MenuItem("Assets/InGameLog/Remove InGameLog")]
    public static void RemoveInGameLog()
    {
        MonoBehaviour mb = GameObject.FindObjectOfType<InGameLog>();
        if (mb != null)
        {
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("Clear", barStyle, GUILayout.Height(50), GUILayout.Width(60)))
        {
            clear();
        }
        ...
        if (GUILayout.Button("Clear On New\nScene Loaded", (clearOnNewSceneLoaded) ? buttonActiveStyle : barStyle,
            GUILayout.Height(50)))
        {
        ...
        if (GUILayout.Button(warningContent, (showWarning) ? buttonActiveStyle : barStyle, GUILayout.Height(50)))
        {
            ...
            if (showWarning)
            {
                var m = GameObject.FindObjectOfType<Camera>();
                //var radialBlurEffect = m.gameObject.GetComponent<RadialBlurEffect> ();
                //radialBlurEffect.enabled = !radialBlurEffect.enabled;
            }
        }
        ...
        for (int i = startIndex; (startIndex + index) < endIndex; i++)
        {
            ...
            {
                GUILayout.BeginHorizontal(((startIndex + index) % 2 == 0) ? evenLogStyle : oddLogStyle,
                    GUILayout.Height(30));
        ...
        if (selectedIndex != -1)
        {
            ...
            if (selectedLog != null)
            {
                string stacktrace = (string.IsNullOrEmpty(selectedLog.stacktrace))
                    ? "stacktrace is empty"
