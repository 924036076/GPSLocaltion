...
namespace ETools
{
    ...
    public class AudioClipPropertyDrawer : PropertyDrawer
    {
        ...
        private Dictionary<ButtonState, Action<SerializedProperty, AudioClip>> _audioButtonStates =
            new Dictionary<ButtonState, Action<SerializedProperty, AudioClip>>
            {
        ...
        public override void OnGUI(Rect position, SerializedProperty prop, GUIContent label)
        {
            ...
            if (prop.objectReferenceValue != null)
            {
                ...
                DrawButton(position, prop,
                    Array.IndexOf(AssetDatabase.GetLabels(prop.objectReferenceValue), _scratchAudioLabel) > -1);
            }
            else
            {
                EditorGUI.PropertyField(position, prop, label, true);
        ...
        private void DrawButton(Rect position, SerializedProperty prop, bool isScratchAudio)
        {
            if (prop.objectReferenceValue != null)
            {
                ...
                if (isScratchAudio)
                {
                    ...
                    GUIContent content =
                        new GUIContent("⁻ˢᶜʳᵃᵗᶜʰ⁻", "This AudioClip Asset is labeled as 'ScratchAudio'");
